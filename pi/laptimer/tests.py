...
class DjangoSettingsTestCase(TestCase):
	...
	def test_gpio_sensor_defaults_to_13(self):
		self.assertTrue(django_settings.exists('gpio_sensor'))
		self.assertEqual(13, django_settings.get('gpio_sensor'))
...
class LapTestCase(TestCase):
	...
	@classmethod
	def setUpClass(cls):
		cls.start = timezone.make_aware(datetime.datetime.now(), timezone.get_default_timezone())
